...
class LoginBloc extends Cubit<LoginState> {
  ...
  Future<void> onLoginWithOAuth(AuthProvider provider) async {
    emit(state.copyWith(socialMediaLoginStatus: SocialMediaLoginStatus.loading));
    try {
      await _authenticationService.signInWithOAuth(provider);
      emit(state.copyWith(socialMediaLoginStatus: SocialMediaLoginStatus.success));
    } catch (_) {
      emit(state.copyWith(socialMediaLoginStatus: SocialMediaLoginStatus.failure));
    }
  }
}
