...
export 'package:causeApiClient/causeApiClient.dart' show LearningResource, LearningResourceTypeEnum;
...
LearningResourceType getResourceTypeFromEnum(LearningResourceTypeEnum type) {
	switch (type) {
		...
		case LearningResourceTypeEnum.READING:
			return LearningResourceType(name: "Reading", icon: CustomIcons.ic_learning);
		case LearningResourceTypeEnum.INFOGRAPHIC:
			return LearningResourceType(name: "Infographic", icon: CustomIcons.ic_report);
		// TODO Are these real?
	...
	return LearningResourceType(name: "Other", icon: FontAwesomeIcons.chalkboardTeacher);
}
...
extension LearningResourceExtension on LearningResource {
	...
	LearningResourceType get type => getResourceTypeFromEnum(learningResourceType);
	IconData get icon => type.icon;

	Cause get cause => causes[0];

	bool isCompletedByUser(CausesUser user) {
		print("Checking is completed: ${user.completedLearningResourceIds.contains(this.id)}");
